...
import { deleteBookAction, saveBookAction } from "../actions";
...
export default function BooksManager({ books, media }: { books: Book[]; media: MediaOption[] }) {
  ...
  const save = async () => {
    ...
    const result = await saveBookAction(id, input);
    setBusy(false);
      ...
      <ul className="flex flex-col gap-3">
        {books.map((b) => (
          <li key={b.id} className="flex items-center gap-5 border border-rule-light bg-white px-5 py-4">
            {b.coverMediaId ? (
              <img src={`/api/media/${b.coverMediaId}`} alt="" className="h-16 w-12 border border-rule-light object-cover" />
            ) : (
            ...
            <button
              ...
              onClick={async () => {
                ...
                if (b.id) await deleteBookAction(b.id);
                router.refresh();
              }}
