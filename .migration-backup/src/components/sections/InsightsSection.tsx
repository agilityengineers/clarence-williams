...
import { getFeedItems, type FeedItem } from "@/lib/rss";
import { Eyebrow } from "./shared";

type InsightsContent = z.infer<typeof insightsSchema>;
type Article = FeedItem & { num: string; dateLabel: string };

export default async function InsightsSection({ content }: { content: InsightsContent }) {
  const raw = await getFeedItems(content.feedUrl);
