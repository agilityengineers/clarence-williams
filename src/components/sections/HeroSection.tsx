...
export default async function HeroSection({ content }: { content: HeroContent }) {
  const t = THEMES[content.theme] ?? THEMES["Midnight Navy"];
  const settings = await getSiteSettings();
  const portraitSrc = content.portraitMediaId
    ? `/api/media/${content.portraitMediaId}`
    : "/assets/portrait-cutout.png";
